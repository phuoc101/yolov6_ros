...
class Yolov6Detector:
    def __init__(self):
        ...
        # Initialize weights
        self.weights = osp.join(ROOT, rospy.get_param("~weights", "weights/yolov6n.pt"))
        print(self.weights)
        # Initialize model
        self.device = rospy.get_param("~device", "gpu")
        self.inferer = Inferer(self.weights, self.device, self.yaml, self.img_size, self.half)

        # Initialize subscriber to Image/CompressedImage topic
        self.img_type, self.img_topic, _ = get_topic_type(
            rospy.get_param("~input_image_topic", "/image_raw"), blocking=True
        )
        self.compressed_input = self.img_type == "sensor_msgs/CompressedImage"
        1
        if self.compressed_input:
            self.img_sub = rospy.Subscriber(self.img_topic, CompressedImage, self.callback, queue_size=1)
        else:
        ...
        self.bounding_boxes_pub = rospy.Publisher(self.output_topic, BoundingBoxes, queue_size=10)
        self.output_img_topic = rospy.get_param("~output_img_topic", "yolov6/detection_image")
        self.output_img_pub = rospy.Publisher(self.output_img_topic, Image, queue_size=10)
    ...
    def img_cb(self, msg):
        ...
        det, img_src = self.inferer.detect(
            frame, self.conf_thres, self.iou_thres, None, self.agnostic_nms, self.max_det, False, False, False, False
        )
        img_ori = img_src
        ...
        if len(det):
            for *xyxy, conf, cls in reversed(det):
                ...
                self.inferer.plot_box_and_label(
                    img_ori,
                    max(round(sum(img_ori.shape) / 2 * 0.003), 2),
                    xyxy,
                    label,
                    color=self.inferer.generate_colors(class_num, True),
                )
    ...
    def summary(self):
        return (
            f"\N{rocket}\N{rocket}\N{rocket} Yolov6 Detector summary:\n"
